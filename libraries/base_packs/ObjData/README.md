...
# Obj Data
  ...
  | 📖️  **Library**  | `@jaisocx/objdata` | 🏷️  **ver.**: `2.1.3` | 

  | 🗓  **Updated**  | Winter 2026 | `Tue Jan 20 06:28:29 CET 2026` | 

  | 📐  **Size**     | 🗂 Folder: 155 KB | 📦 Tarball: _ KB | 📋 .js: 5 KB | 
...
## 💡 The aim of the setup
  ...
  This is just another try to implement a binary data format 
...
## This format info
  ...
  The objdata format is not being parsed by chars comparisons, 
  but like in the good old style packets prefixes 
  fixed positions and lengths binary fields 
  with numeric values kept. 



## For what 

  Since the JSON format is hardly editable 
  when a .json document is big, 
  and the depth of the nested objects grows, 
  then the usage of the visual tool to edit objects becomes a must, 
  and contents preview can be done by viewing json text 
  when serialized to JSON format, 
  or visualized with a JSTree js browser tool. 
  The JS object remains still the same.
